...
function Voting({ socket }) {
  const { isHost, players, updatePlayers, joinCode, userName, prompt} = useGameRoom();
          ...
          <div>
            ...
            <div className="flex-row justify-center">
              {players.map((player, index) => (
                <div key={index} className={`${selected == player.username ? 'bg-cyan-200' : 'bg-slate-200'} m-4`} onClick={() => selectResponse(player.username)}>
            ...
            <button
              ...
              onClick={sendVote}
            >{selected == "" ? 'Who is the imposter?' : `${selected} is the imposter!`}</button>
        </div>
        )}
          ...
          <div>
            ...
            <div className="flex-row justify-center">
              ...
              {Object.entries(tallyVotes).map(([votee, value]) => (
                <div key={votee} className="font-bold rounded-lg py-2 px-5 inline-block border border-black shadow shadow-lg mb-4 mx-1"
                ...
                <div key={key}>
                  {key == "votes" && ( 
                    <p>Number of Votes: {value2}</p>
                  )}
                  {key == "voters" && ( 
                    <p>Voters: {value2.join(", ")} </p>
                  )}
                </div>
